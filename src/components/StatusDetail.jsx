...
const StatusDetail = ({ statusId, updatedBookmarks, statusText}) => {
    ...
    const sendArchiveRequest = async () => {
        setLoading(true);
        try {
            const response = await fetch(`https://cargozhibekzholy.kz:3001/api/archive/${userId}/archive`, {
                method: 'POST',
                headers: {
    ...
    const removeBookmark = async (trackNumber) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`https://cargozhibekzholy.kz:3001/api/bookmark/${userId}/delete/${trackNumber}`, {
                    method: 'DELETE',
                });
