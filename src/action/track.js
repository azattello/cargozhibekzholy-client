...
export const addTrack = async (track, status, date) => {
  try {
    const response = await axios.post('https://cargozhibekzholy.kz:3001/api/track/addTrack', {
      track,
      status,
      date
    });
...
export const excelTracks = async (tracks, status, date) => {
  try {
      ...
      const response = await axios.post('https://cargozhibekzholy.kz:3001/api/track/addExcelTrack', {
          tracks,
          status,
          date
      });
